...
const SECTION_HEAD = "text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#0f766e] pb-1.5 mb-2 border-b border-zinc-300";

/** Resume 2: light two-column – profile header, left: About Me + Education + Skills, right: Experience + Projects. */
    ...
    <article className={`${DOCUMENT_CLASS} max-w-4xl flex flex-col overflow-visible bg-[#fafaf9] print:bg-[#fafaf9] border border-zinc-200 print:border-zinc-300`}>
      <div className="w-full flex flex-row flex-wrap items-start justify-between gap-4 px-4 sm:px-6 pt-5 pb-4 border-b border-zinc-200">
        ...
        <div className="space-y-1.5 text-xs text-zinc-700 text-right">
          ...
          {data?.email && (
            <p className="flex items-center gap-2 break-all justify-end">
      ...
      <div className="w-full flex flex-col md:flex-row print:flex-row flex-1 min-h-0">
        ...
        <div className="w-full md:w-[62%] print:w-[62%] min-h-0 flex flex-col px-4 sm:px-5 py-4 bg-[#fafaf9] print:bg-[#fafaf9]">
          {experienceEntries.length > 0 && (
            <section className="mb-4">
              <h2 className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#0f766e] pb-1 mb-2 border-b border-zinc-300">Experience</h2>
              <div className="space-y-3">
                {experienceEntries.map((entry, i) => (
                  <div key={i}>
                    <p className={`text-[11px] font-bold ${TEAL} leading-snug`}>{entry.jobTitle || "Role"}</p>
                    {entry.company && <p className="text-[11px] text-zinc-600 leading-snug">{entry.company}</p>}
                    {entry.datesOrLocation && <p className="text-[10px] text-zinc-500 mt-0.5">{entry.datesOrLocation}</p>}
          ...
          {projectsList.length > 0 && (
            <section className="mb-4">
              <h2 className={`flex items-center gap-2 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#0f766e] pb-1 mb-2 border-b border-zinc-300`}>
                <FolderOpen size={12} className="shrink-0 text-[#0f766e]" /> Projects
              </h2>
